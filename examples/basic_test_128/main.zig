...
const Chip = lc4k.LC4128V_TQFP144;
...
pub fn main() !void {
    ...
    var jed_file = try std.fs.cwd().createFile("basic_test_128.jed", .{});
    defer jed_file.close();
    try Chip.write_jed(results.jedec, jed_file.writer(), .{});

    var svf_file = try std.fs.cwd().createFile("basic_test_128.svf", .{});
    defer svf_file.close();
    try Chip.write_svf(results.jedec, svf_file.writer(), .{});

    var report_file = try std.fs.cwd().createFile("basic_test_128.html", .{});
