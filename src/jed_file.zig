...
pub fn write(comptime Device: type, allocator: std.mem.Allocator, f: JedecFile, writer: anytype, options: WriteOptions) !void {
    var w = @import("checksum_writer.zig").checksumWriter(u16, allocator, writer);

    try w.writeByte(0x2); // STX
    try w.print("{s} configuration generated by https://github.com/bcrist/Zig-LC4k*{s}", .{ @tagName(Device.device_type), options.line_ending });
